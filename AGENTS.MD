...
- **MCP Server System for Hermes Agent**: Standard JSON-RPC 2.0 HTTP/SSE (`/mcp`, `/api/mcp/sse`) & Stdio (`mcp_server.py`) interface exposing `get_models`, `set_model_mapping`, `get_system_config`, `update_system_config`, `get_metrics`, and `control_circuit_breaker`.
